...
We provided user-friendly visualization function `lambda_tuning_viz`, 9 measurements were tested in each lambda, a detailed description is provided below.

- Percentage : Percentage of features were selected in each lambda.
- Prevalence : The medium value of selected feature in each lambda.
- Feature_number : The number of selected feature in each lambda.
...
- pairwiseMCC : Matthews correlation coefficient of selection vector between k-fold CV.
...
```
We strongly recommend that choose the lambda at point which form gentle slope to the pull-up in BCE loss curve, this is the critical point where the regularization term is stronger than the loss term. So we provided `lambda_decision`.
