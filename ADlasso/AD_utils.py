...
def auto_scale(X_input, X_raw, Y, step=50, device='cpu', training_echo=False,
    ...
    for lmbd in exam_range:
        exam_res = ADlasso(lmbd=lmbd, device=device, echo=training_echo, max_iter=max_iter, tol=tol, lr=lr, alpha=alpha, epsilon=epsilon)
...
def lambda_tuning(X_input, X_raw, Y, lmbdrange, k_fold, outdir, device='cpu', training_echo=False,
    ...
    for i in range(n_lambda):
        ...
        for train_ix, test_ix in kfold.split(X_input, y):
            ...
            lambd = lmbdrange[i]
            examined_lambda = ADlasso(lmbd = lambd, device = device, echo = training_echo, max_iter=max_iter, tol=tol, lr=lr, alpha=alpha, epsilon=epsilon)
